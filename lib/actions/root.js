(() => {
  ...
  class Root extends Action {
    ...
    do(sb) {
      ...
      sb.addRandom(roots.filter(getCandidates, sb));

    }

    canDo(sb) {

      return super.canDo(sb) && roots.some(getCandidates, sb);
  ...
  function getCandidates(root) {
    ...
    return this.freeSpace >= root.length &&
      startsWithVowel(root) !== this.endsWithVowel;
